    ...
    return OrientationBuilder(
      builder: (context, orientation){
        if(orientation == Orientation.portrait){
          ...
          ScreenUtil.init(context, designSize: Size(844, 390));
        }
        ...
        return GetMaterialApp(
          ...
          // theme: CustomTheme.lightTheme, // 使用自定义的浅色主题
          // darkTheme: CustomTheme.darkTheme, // 使用自定义的深色主题
...
class _MyHomePageState extends State<MyHomePage> with WidgetsBindingObserver {
          ...
          MediaQuery.removePadding(
            ...
            child: Container(
              ...
              child: ListView(
                ...
                children: [
                  SizedBox(height: ScreenUtilHelper.setHeight(50),),
                  _mainTitleClock(context),
                  SizedBox(
                    height: ScreenUtilHelper.setHeight(20),
                  ),
                  Obx(() => _mainListWidget("睡眠", "开启后改时间段内，屏幕亮度将会有所降低，在结束时恢复，并自动开启天气闹钟。", isWeatherAlarmEnabled.value,  (value) => _setWeatherAlarmEnabled(value!),(value) => _showDialog(true), true)),
                  Obx(() => _mainListWidget("沉浸式通知", "开启后将在全屏显示时间时，沉浸式显示通知内容。", isNotificationReminder.value, (value) => _setNotificationReminder(value), (value) => _showDialog(false), false)),
                  Obx(() => _mainListWidget("每日一言", "开启后将在全屏显示时间时，沉浸式显示通知内容。", isAWordDay.value,  (value) => { _setAWordDay(value!)},(value) => _showDialog(false), false)),
  ...
  //   });
  //
  //   homePageController.updateData(weatherModel);
  //   homePageController.updateData(lunisolarCalendarModel);
  // }
  ...
  fetchWeather() async {
    ...
    // Update the observable variables in the controller directly
      ...
      builder: (context) =>  SingleChildScrollView(
        ...
        child: Column(
              ...
              child: Obx(() => TimePicker(
                  ...
                  decoration: TimePickerDecoration(
                    ...
                    sweepDecoration: TimePickerSweepDecoration(
                      ...
                      pickerColor:
                          _isSleepGoal.value ?  const Color(0xFFFF5252) : const Color(0xFF3CDAF7),
                    ),
                    initHandlerDecoration: TimePickerHandlerDecoration(
                      color: _isSleepGoal.value ?  const Color(0xFFFF5252) : const Color(0xFF3CDAF7),
                    ...
                    clockNumberDecoration: TimePickerClockNumberDecoration(
                      defaultTextColor:  _isSleepGoal.value ?  const Color(0xFFFF5252) : const Color(0xFF3CDAF7),
          ...
          width: ScreenUtilHelper.setWidth(300),
               margin:  EdgeInsets.only(top: ScreenUtilHelper.setWidth(15)),
               padding:  EdgeInsets.all(ScreenUtilHelper.setWidth(16.0)),
            ...
            BoxShadow(
              ...
              child: Obx(() =>  Text( _isSleepGoal.value
                    ...
                    color:  _isSleepGoal.value ?  const Color(0xFFFF5252) : const Color(0xFF3CDAF7),
                    fontSize: ScreenUtilHelper.setSp(22),
  ...
  _mainListWidget(String title, String subTitle, bool values,  Function(bool?) onTap, Function(bool) OnTap, bool isShow){
    return InkWell(
      onTap: () => OnTap(isShow),
      child: Container(
        // width: ScreenUtilHelper.setWidth(400),
        margin: EdgeInsets.fromLTRB(ScreenUtilHelper.setWidth(20), ScreenUtilHelper.setHeight(10), ScreenUtilHelper.setWidth(20), ScreenUtilHelper.setHeight(10)),
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(ScreenUtilHelper.setHeight(20))
        ),
        child:
              ListTile(
                          title: Text(title, style: TextStyle(color: Color(0xFF323031), fontSize:  ScreenUtilHelper.setSp(20), fontWeight: FontWeight.bold)),
                          subtitle: Padding(
                  ...
                  trailing:         Container(
                    decoration:  BoxDecoration(
                        ...
                        boxShadow: [     BoxShadow(
                          color: values ? const Color.fromRGBO(71,234,139, 0.3) : const Color.fromRGBO(255,70,80, 0.3),
  ...
  _timeWidget(String s, PickedTime time, Icon icon) {
    ...
    return Container(
      ...
      margin:  EdgeInsets.fromLTRB(ScreenUtilHelper.setWidth(0),  ScreenUtilHelper.setHeight(20), ScreenUtilHelper.setWidth(0), ScreenUtilHelper.setHeight(0)),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(25.0),
                  boxShadow: [
  ...
  void _addSleepAlarmClock() {
    inBedTimeTimestampInSeconds.value =Utils().calculateTimestamp(Time(inBedTime.value.h, inBedTime.value.m));
    outBedTimeTimestampInSeconds.value =Utils().calculateTimestamp(Time(outBedTime.value.h, outBedTime.value.m));

    isWeatherAlarmEnabled.value = !isWeatherAlarmEnabled.value;

    print("value-----${isWeatherAlarmEnabled.value}");

    initAppData();

    SpUtils.setBool("isWeatherAlarmEnabled", isWeatherAlarmEnabled.value); //开启天气时钟
  ...
  Widget? clickableButtonMethod(BuildContext context, String text, VoidCallback onTap, ) {
    return InkWell(
      ...
      child: Container(
        ...
        margin: EdgeInsets.fromLTRB(ScreenUtilHelper.setWidth(25),  ScreenUtilHelper.setHeight(20), ScreenUtilHelper.setWidth(25), ScreenUtilHelper.setHeight(25)),
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(20),
        ),
        padding: EdgeInsets.fromLTRB(ScreenUtilHelper.setWidth(15),  ScreenUtilHelper.setHeight(15), ScreenUtilHelper.setWidth(15), ScreenUtilHelper.setHeight(15)),
        child: Center(
          child: Text(text, style: TextStyle(fontSize: ScreenUtilHelper.setSp(15), fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
  ...
  void initAppData() {
    isNotificationReminder.value = SpUtils.getBool("isNotificationReminder") ?? false;
 ...
 void _startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      currentTimestampInSeconds.value =Utils().calculateTimestamp(Time(DateTime.now().hour, DateTime.now().minute));

      if(currentTimestampInSeconds <= inBedTimeTimestampInSeconds.value) {
        print("进入夜间模式");
      }else if(currentTimestampInSeconds <= outBedTimeTimestampInSeconds.value){
        print("进入后半夜夜间模式");
      }else {
